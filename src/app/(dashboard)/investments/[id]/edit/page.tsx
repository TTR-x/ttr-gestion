...
export default function EditInvestmentPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        if (investment) {
            form.reset({
                ...
                timeframeMonths: investment.timeframeMonths,
    ...
    async function onSubmit(values: InvestmentFormValues) {
        if (!currentUser?.uid || !currentUser.displayName || !businessId || !activeWorkspaceId || !investment) {
            toast({ variant: "destructive", title: "Erreur", description: "Action non autorisée ou investissement introuvable." });
            return;
        }
        showLoader();
        try {
            ...
            await syncService.updateInvestment(updatedInvestment);

            toast({
                title: "Investissement mis à jour",
                description: `Le projet "${values.name}" a été modifié avec succès.`,
            });
            ...
            toast({
                ...
                description: error.message || "Impossible de modifier l'investissement.",
            });
        } finally {
            hideLoader();
        }
    ...
    if (!investment) {
        return <div className="p-8 text-center"><Loader2 className="h-8 w-8 animate-spin mx-auto" /><p className="mt-2 text-muted-foreground">Chargement...</p></div>;
    }
    ...
    return (
        <div className="space-y-8 max-w-4xl mx-auto">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-3xl font-headline font-semibold tracking-tight">Modifier l'Investissement</h1>
            </div>
