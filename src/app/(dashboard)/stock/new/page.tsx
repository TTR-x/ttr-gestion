...
export default function NewStockItemPage() {
  ...
  async function onSubmit(values: StockItemFormValues) {
    ...
    try {
      ...
      await syncService.createStockItem(newItem);

      router.push("/stock");
      toast({
        title: "Article ajouté au stock",
        description: `L'article "${values.name}" a été enregistré.`,
      });
      ...
      toast({
        ...
        description: error.message || "Impossible d'enregistrer l'article.",
      });
    } finally {
      hideLoader();
    }
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto min-h-screen pb-24">
      <div className="flex items-center gap-4">
        <Button variant="outline" size="icon" asChild>
          <Link href="/stock" onClick={showLoader}><ArrowLeft className="h-4 w-4" /></Link>
        </Button>
        <h1 className="text-3xl font-headline font-semibold tracking-tight">Nouvel Article en Stock</h1>
      </div>

      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle>Détails de l'article</CardTitle>
          <CardDescription>Remplissez les informations ci-dessous pour ajouter un nouvel article à votre inventaire.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Quantité initiale en stock</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          min="0"
